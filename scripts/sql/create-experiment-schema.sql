...
-- 테이블: workerNodes, distributedTasks, distributedExperiments
...
DO $$ BEGIN
  CREATE TYPE distributed_experiment_status AS ENUM ('draft', 'running', 'analyzing', 'completed');
EXCEPTION
  WHEN duplicate_object THEN null;
END $$;
...
-- 4. distributedExperiments 테이블 (실험 DB 전용)
CREATE TABLE IF NOT EXISTS "distributedExperiments" (
  id SERIAL PRIMARY KEY,
  name VARCHAR(255) NOT NULL,
  description TEXT,
  "variableConfig" TEXT NOT NULL,
  "testCountPerCombo" INTEGER DEFAULT 10 NOT NULL,
  "successThreshold" INTEGER DEFAULT 90,
  status distributed_experiment_status DEFAULT 'draft' NOT NULL,
  generation INTEGER DEFAULT 0 NOT NULL,
  "totalCombinations" INTEGER DEFAULT 0,
  "testedCombinations" INTEGER DEFAULT 0,
  "eliteCombinations" TEXT,
  "createdAt" TIMESTAMP DEFAULT NOW() NOT NULL,
  "startedAt" TIMESTAMP,
  "completedAt" TIMESTAMP
);

-- distributedExperiments 인덱스
CREATE INDEX IF NOT EXISTS idx_distributed_experiments_status ON "distributedExperiments" (status);

-- Foreign Key (optional - distributedTasks.experimentId -> distributedExperiments.id)
-- ALTER TABLE "distributedTasks" ADD CONSTRAINT fk_experiment
--   FOREIGN KEY ("experimentId") REFERENCES "distributedExperiments"(id) ON DELETE SET NULL;

-- 완료 메시지
SELECT 'Experiment DB Schema Created Successfully!' as message;
